...
{
    var date_test = formatearFecha(new Date().toString());
...
function traerTop10TiposVentas()
{
    ...
    var sql = "elect first 10 m.d_tipo_venta || '(' || m.c_tipo_venta || ')' tipo_venta, sum(vr_subtotal) total from h_ventas h, m_tipos_venta m where h.c_tipo_venta = m.c_tipo_venta and h.f_factura = '" + date_test + "' group by 1 order by total desc";
    xmlQueryDB(sql, id_query, 1, false, ruta);
    var ar_status = getStatusDB(id_query);
    var size = ar_status['numrows'] - 1;
    var ar_tipos = [], ar_ventas = [];
